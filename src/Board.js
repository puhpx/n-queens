 ...
 */
    ...
    hasAnyMajorDiagonalConflicts: function() {
      ...
      var rowIndex = size - 2;
      var colIndex = 0;
      for (var i = 2 - size; i < size - 2; i++) {
        if (this.hasMajorDiagonalConflictAt(colIndex, rowIndex) === true) {
          return true;
        } else if (rowIndex === 0) {
          colIndex++;
        } else {
    ...
    hasMinorDiagonalConflictAt: function(minorDiagonalColumnIndexAtFirstRow, rowIndex) {
      ...
      while ((colIndex !== -1) && (rowIndex !== this.attributes.n)) {
        if (this.attributes[rowIndex][colIndex] === 1) {
          count++;
          if (count === 2) {
            return true;
    ...
    hasAnyMinorDiagonalConflicts: function() {
      ...
      var colIndex = size - 1;
      for (var i = 2 - size; i < size - 2; i++) {
        if (this.hasMinorDiagonalConflictAt(colIndex, rowIndex) === true) {
          return true;
        } else if (rowIndex === 0) {
          colIndex--;
        } else {
